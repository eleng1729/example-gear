...
function [imgsort, infosort, overlaysort, images, infos, overlays] = readDicomSeriesOverlays(sourcedir)
images = [];
overlays = [];
...
[nRow, nCol, nCh] = size(tmp);
if nCh>1
   % This is RGB Data!
   warning('RGB Data - not tested!');
end
images = zeros(nRow, nCol, numfiles);
overlays = zeros(nRow, nCol, numfiles);
for idx=1:numfiles
    % Not every slice has an overlay!
    [images(:,:,idx), ~, ~, OVERLAYS] = dicomread(infos{idx});
    if ~isempty(OVERLAYS)        
        overlays(:,:,idx) = OVERLAYS;
    end
end
...
try
    ...
    if (nAcqs>nBVals)
        ...
        overlaysort = zeros(nRow, nCol, nSlices, nEchoes, nAcqs);
        for idx=1:numfiles
            % Find indices
            ...
            overlaysort(:,:,slidx,echoidx,acqidx) = overlays(:,:,idx);            
        ...
        overlaysort = zeros(nRow, nCol, nSlices, nEchoes, nBVals);
        infosort = cell(nSlices,nEchoes,nAcqs);
        for idx=1:numfiles
            % Find indices
            slidx = uslices==slicepos(idx);
            ...
            overlaysort(:,:,slidx,echoidx,bidx) = overlays(:,:,idx);
