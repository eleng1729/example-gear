...
% WOrm 3 has two problems: crosshairs are off a bit, and cannot scale plot

function worm4(img3d)
fprintf('Worm4 v1\n');
...
set(gcf, 'name', sprintf('Worm4 time %d/%d',  apd.timept, apd.NtimePts));
...
data1D = squeeze(apd.img3d(apd.slice(1),apd.slice(2),apd.slice(3),:));
plot(data1D);
set(apd.axisPlot, 'xlim', [0.5, apd.NtimePts+.5]);
set(apd.axisPlot, 'ylim', apd.clim);
...
switch get(figH, 'SelectionType')
    ...
    case 'open' % double-clickw
        ...
        elseif(gca == apd.axisPlot)
            % Double click time course. Rescale
            fprintf('Double click on plot\n')
            data1D = squeeze(apd.img3d(apd.slice(1),apd.slice(2),apd.slice(3),:));
            fprintf('Limits were [%f, %f]\n', apd.clim(1), apd.clim(2));
            apd.clim(1) = min(data1D);
            apd.clim(2) = max(data1D);
            apd.InitClim = apd.clim;
            fprintf('Changed to [%f, %f]\n', apd.clim(1), apd.clim(2));
