...
fprintf('clim [%.1f %1f], W/L = %f/%f\n', ...
    apd.clim(1), apd.clim(2),...
    apd.clim(2)-apd.clim(1), mean(apd.clim(:)));
return
...
window0 = apd.initClim(2)-apd.initClim(1);
level0 = mean(apd.initClim(:));

% Change them
scaleW = .0001 * apd.max;
scaleL = 0.0001 * apd.range;
level = level0 + dy .* scaleL;
%level = max(level, 0);
%window = window .* (1/(1+dx * scaleW));
%window = max(apd.range*.01, 0);
%dx
window = window0 + dx*scaleW;
