...
function [S0, R, S0ci, Rci, Sinf, Sinfci, Rsq] = fit_to_exp_decay_3param(...
...
beta0(2) = Rinitial; % R
beta0(3) = 0;
lowBounds = [0.0 Rinitial/100 0];
upBounds = [100 Rinitial*100 max(abs(yvals(:)))];
...
if ( bShowFitting)
    ...
    title(sprintf(...
        '2-param R Fit: S0=%.1f+/-%.1f, R=%.1f+/-%.1f, 1/R=%.1f+/-%.1f, Sinf=%.1f\n',  ...
        S0, S0ci, R, Rci, 1/R, Rci/R^2, Sinf));
    legend('data', 'fit')
